...
_HUB_USES_PATTERN = re.compile(
    r"^(?P<indent>\s*)uses:\s*(?P<repo>[^/\s]+/[^/\s]+)/\.github/workflows/hub-ci\.yml@(?P<ref>\S+)\s*$",
    re.MULTILINE,
)
...
def render_caller_workflow(language: str, hub_workflow_ref: str | None = None) -> str:
...
def render_dispatch_workflow(language: str, dispatch_workflow: str, hub_workflow_ref: str | None = None) -> str:
    templates_dir = hub_root() / "templates" / "repo"
    if dispatch_workflow == "hub-ci.yml":
        if not language:
            raise ValueError("language is required for hub-ci.yml rendering")
        return render_caller_workflow(language, hub_workflow_ref=hub_workflow_ref)
