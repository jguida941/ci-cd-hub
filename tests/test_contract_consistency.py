...
# Per ADR-0024: thresholds are bundled into threshold_overrides_yaml, not inputs.
...
def test_python_caller_template_matches_workflow():
    ...
    # Per ADR-0024: thresholds/metadata are hardcoded in with:, not dispatch inputs.
...
def test_java_caller_template_matches_workflow():
    ...
    # Per ADR-0024: thresholds/metadata are hardcoded in with:, not dispatch inputs.
    # run_docker is now a dispatch input; docker settings remain config defaults.
    allowed_missing = {
        "docker_compose_file",
        "docker_health_endpoint",
        "artifact_prefix",
