...
| --- | --- | --- |
| Action pinning & workflow integrity | [x] | `scripts/check_workflow_integrity.py`, all workflows use SHA pins. |
| Supply-chain verification | [x] | Checksums enforced in `scripts/install_tools.sh`; release job verifies SBOM/VEX/provenance (see `.github/workflows/release.yml:974-1239`). |
| Evidence bundle signing | [x] | `scripts/sign_evidence_bundle.sh` now fails fast on verification errors; release step re-verifies with repo-scoped identity. |
| Dynamic repo registry | [x] | `config/repositories.yaml` + `scripts/load_repository_matrix.py` feed the matrix via heredoc output (`release.yml:92-103`). |
...
| Rekor/SLSA/SBOM evidence | [x] | Stored under `artifacts/signed-evidence/` with cosign/rekor proofs. |
| Test automation | [x] | `pytest` suite (85 tests) green locally with pinned dependencies. |
...
1. [x] Trigger v1.0.10 release workflow; capture results of proxy enforcement and determinism follow-up run.
