...
- Add regression tests for fixture thresholds.
 - Interim: `cihub-test-python-pyproject` and `cihub-test-python-src-layout` use `max_pip_audit_vulns: 1` in repo `.ci-hub.yml` (now on main) to pass `pip_audit` until a fix is available.
