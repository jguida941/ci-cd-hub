...
# Cross-platform file mtime to YYYY-MM-DD
mtime() {
  local file="$1"
...
extract_metadata() {
  ...
  if [[ -f "$file" ]]; then
    ...
    if grep -Fq "**Status:" "$file" 2>/dev/null; then
      status=$(grep -Fm1 "**Status:" "$file" | sed 's/.*Status:[[:space:]]*//' | cut -d' ' -f1)
    fi

    # Try to extract date from snapshot or last-updated line
    if grep -Fq "Last updated:" "$file" 2>/dev/null; then
      last_reviewed=$(grep -Fm1 "Last updated:" "$file" | sed 's/.*Last updated:[[:space:]]*//' | cut -d' ' -f1)
    elif grep -Fq "**Snapshot:" "$file" 2>/dev/null; then
      last_reviewed=$(grep -Fm1 "**Snapshot:" "$file" | sed 's/.*Snapshot:\*\*//' | sed 's/^[[:space:]]*//' | cut -d' ' -f1)
    fi

    # If still unknown, use file mtime as a proxy to avoid placeholder dates
    if [[ "$last_reviewed" == "Unknown" ]]; then
      last_reviewed="$(mtime "$file")"
    fi
  fi
...
# Fail early if any placeholder dates remain
if grep -q "Unknown" "$INDEX_FILE"; then
  log "ERROR: docs/index.md still contains placeholder review dates (Unknown)."
  exit 1
fi
