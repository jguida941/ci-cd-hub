...
usage: cihub [-h] [--version]
             {detect,preflight,doctor,scaffold,smoke,check,verify,ci,run,report,docs,adr,config-outputs,discover,hub-ci,new,init,update,validate,setup-secrets,setup-nvd,fix-pom,fix-deps,sync-templates,config}
             ...
...
usage: cihub discover [-h] [--json] [--hub-root HUB_ROOT] [--repos REPOS]
                      [--run-group RUN_GROUP] [--github-output]
...
```
usage: cihub hub-ci badges [-h] [--check] [--output-dir OUTPUT_DIR]
