...
- [x] `require_run_or_fail` policy implemented (tools that must run can fail CI)
- [x] GateSpec registry created (single source of truth for gates.py + reporting.py)
- [x] Self-validation after `cihub ci` catches summary/report contradictions
- [x] Contract tests ensure gate evaluation matches summary rendering
